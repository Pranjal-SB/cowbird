"""Hits real inboxkitten.com. Skipped by default; this is the canary."""

import pytest
from cowbird.transport import Transport
from cowbird_inboxkitten import InboxKitten

pytestmark = pytest.mark.live


@pytest.fixture
async def provider():
    http = Transport("inboxkitten")
    try:
        yield InboxKitten(http)
    finally:
        await http.aclose()


async def test_generate_and_list_against_the_real_service(provider):
    address = await provider.generate()
    assert await provider.list(address) == []
